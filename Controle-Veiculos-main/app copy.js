...
// Substitua a rota antiga por esta (usa query já existente no app)
app.get('/admin/stats', isAuthenticated, isAdmin, async (req, res) => {
  if (typeof query !== 'function') {
    console.error('[admin/stats] helper "query" não encontrado.');
    return res.status(500).send('Erro interno: helper de BD não encontrado.');
  }
  ...
  const safeRows = rows => Array.isArray(rows) ? rows.map(safe) : [];
  ...
  const motorista  = req.query.motorista || null;

  // **USAR nomes de coluna sem alias** para evitar "Unknown column 'a.data_hora'"
  const whereParts = [];
  const whereParams = [];
  ...
  try {
    ...
    `);
    const kpi = (Array.isArray(kpiRows) && kpiRows.length) ? safe(kpiRows[0]) : {
      total_veiculos: 0, total_motoristas: 0, total_usos: 0, total_multas: 0, total_valor_multas: 0, total_reembolsos: 0
    };

    // 2) Abastecimentos agregados (usar DATE(data_hora) sem alias)
    ...
    const abastecimentos_totais = safe(abRows && abRows[0] ? abRows[0] : {});

    // 3) KM total (uso_veiculos) — opcional filtro por veículo
    const kmParams = [];
    ...
    // 4) Litros e gasto no período (reutiliza abastecWhere)
    ...
    // 5) Top veículos por gasto no período
    ...
    // 6) Manutenções — totais e por veículo (filtrar por realizado_em)
    const manutWhere = [];
    const manutParams = [];
    ...
    const manutRows = await query(`
      ...
      ${manutWhereClause}
    `, manutParams);
    const manut_totais = safe(manutRows && manutRows[0] ? manutRows[0] : {});
    ...
    // 7) Multas por motorista e por tipo (aplica filtros corretos: coluna 'data' na tabela multas)
    const multasWhere = [];
    const multasParams = [];
    ...
    if (motorista) { multasWhere.push("motorista = ?"); multasParams.push(motorista); }
    ...
    // 8) Geofences (tratamento defensivo)
    ...
    try {
      ...
      geofences = (grows || []).map(r => {
        ...
        try {
          ...
          else if (Buffer && Buffer.isBuffer(r.geojson)) {
            const s = r.geojson.toString('utf8');
            parsed = s ? JSON.parse(s) : null;
          } else if (typeof r.geojson === 'object') parsed = r.geojson;
          else parsed = null;
        } catch (e) {
          console.warn('[admin/stats] geojson parse failed for geofence id=', r.id, e.message);
          parsed = null;
        }
        return { id: r.id, nome: r.nome, descricao: r.descricao, geojson: parsed };
      });

      // tentativa de violações (vai falhar em DB sem spatial support — capturado)
      ...
      console.warn('[admin/stats] spatial queries skipped:', e && e.message);
      geofences = []; geofence_violations = [];
    }

    // 9) pontos recentes (limite 200)
    ...
    // 10) Abastecimentos por dia (período)
    const moment = require('moment');
    ...
    const motoristasList = await query(`SELECT id, nome FROM motoristas ORDER BY nome`);

    // montar stats
    const stats = {
      kpi_frota: kpi,
      abastecimentos_totais: safe(abastecimentos_totais),
      km_total,
      litros_total,
      consumo_medio_l_km,
      custo_por_km,
      top_veiculos_abastecimento: safeRows(topVeicRows),
      manut_totais: safe(manut_totais),
    ...
    return res.render('admin-stats', {
      ...
      user: req.user,
      stats,
      filters: { start: start || '', end: end || '', veiculo_id: veiculo_id || '', motorista: motorista || '' },
